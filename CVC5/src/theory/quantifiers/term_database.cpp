...
bool TermDb::inRelevantDomain(TNode f, unsigned i, TNode r)
{
  ...
  std::map<Node, std::map<unsigned, std::vector<Node> > >::iterator it =
      d_func_map_rel_dom.find(f);
  if (it != d_func_map_rel_dom.end())
  {
    std::map<unsigned, std::vector<Node> >::iterator it2 = it->second.find(i);
    if (it2 != it->second.end())
    {
      return std::find(it2->second.begin(), it2->second.end(), r)
             != it2->second.end();
    }
    else
    {
...
Node TermDb::evaluateTerm(TNode n, bool useEntailmentTests, bool reqHasTerm)
{
  std::map<TNode, Node> visited;
  std::vector<Node> exp;
...
bool TermDb::isTermEligibleForInstantiation(TNode n, TNode f)
{
  if (options::instMaxLevel() != -1)
  {
    if (n.hasAttribute(InstLevelAttribute()))
    {
      ...
      {
        Trace("inst-add-debug") << "Term " << n << " has instantiation level "
...
bool TermDb::reset(Theory::Effort effort)
{
  ...
  if (options::termDbMode() == options::TermDbMode::RELEVANT)
  {
    ...
    while (!eqcs_i.isFinished())
    {
      ...
      Node first;
      // TODO: ignoring singleton eqc isn't enough, need to ensure eqc are
      // relevant
      eq::EqClassIterator eqc_i = eq::EqClassIterator(r, ee);
      while (!eqc_i.isFinished())
      {
  ...
  {
    Trace("quant-ho") << "TermDb::reset : compute equal functions..."
                      << std::endl;
