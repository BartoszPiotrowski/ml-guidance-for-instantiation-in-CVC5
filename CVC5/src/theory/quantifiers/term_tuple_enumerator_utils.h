#ifndef TERM_TUPLE_ENUMERATOR_UTILS_H_28202
#define TERM_TUPLE_ENUMERATOR_UTILS_H_28202
#include <iostream>
#include <iterator>
#include <vector>
...
template <typename T>
Cvc5ostream& operator<<(Cvc5ostream& out, const std::vector<T>& v)
{
  out << "[ ";
...
class [[nodiscard]] ImmutableVector
{
 ...
 public:
  ...
  {
    if (_data != nullptr) ++(_data->count);
  }

  ImmutableVector<T, HashFun, Eq>& operator=(
      const ImmutableVector<T, HashFun, Eq>& ls)
  {
    decrease();
    ...
    const ImmutableVector<T, HashFun, Eq>& other) const
{
  ...
  for (size_t i = 0; i < _size; ++i)
    if (!Eq()(_data->elements[i], other._data->elements[i])) return false;
  return true;
}
    ...
    : public std::iterator<std::forward_iterator_tag, T>
{
 public:
  ...
  const_ImmutableVectorIterator& operator=(
      const const_ImmutableVectorIterator<T, HashFun, Eq>& rhs)
  {
...
{
  o << "[";
  for (size_t i = 0; i < es.size(); ++i) o << (i ? " " : "") << es[i];
  return o << "]";
}

}  // namespace cvc5
#endif /* TERM_TUPLE_ENUMERATOR_UTILS_H_28202 */
